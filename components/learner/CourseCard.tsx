...
export default function CourseCard({ course, href }: CourseCardProps) {
  return (
    <article className="rounded-xl border border-wire-border bg-level-1 p-5 transition hover:border-brand-navy hover:shadow-sm">
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <div className="flex flex-wrap gap-2">
          {course.format.map((entry) => (
            <span key={entry} className="rounded-full border border-wire-border px-2.5 py-1 type-caption text-muted">
              {entry}
            </span>
          ))}
        </div>
      ...
      <p className="mt-2 type-body-sm text-muted">{course.provider}</p>

      <div className="mt-3 flex flex-wrap items-center gap-3 type-body-sm">
      ...
      <div className="mt-4 text-right">
        <Link href={href} className="type-body-sm font-semibold text-primary-loud">
          View Details →
        </Link>
      </div>
    </article>
