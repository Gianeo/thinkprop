  ...
  if (!item) {
    return (
      <div className="min-h-screen bg-level-0 p-8">
        <p className="text-sm text-muted">Compliance item not found.</p>
      </div>
    )
  }
      ...
      <main className="animate-in fade-in duration-200 md:ml-60 pt-16 md:pt-0">
        <div className="space-y-8 p-8">
          ...
          <section>
            ...
            <p className="mt-2 text-sm text-muted">Expires on {item.expiryDate}</p>
          </section>

          <section className={`rounded-xl border p-6 ${panelClasses}`}>
            <div className="flex items-start gap-3">
              <AlertTriangle className={`h-5 w-5 ${countdownClass}`} />
              <p className="text-sm text-default">{item.consequence}</p>
            </div>
          ...
          <p className="text-sm text-muted">Issued by: {item.requirementBody}</p>
